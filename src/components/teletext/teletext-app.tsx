...
const initialContent = {
  ...
  ideas: [
    { title: "Teletext Job Board", status: "new", note: "Ultra-fast board for niche AI/marketing roles. Keyboard nav only." },
    { title: "Ad-Intel Tuner", status: "prototyping", note: "Tune variables (ICP, hook, angle) and auto-generate creatives from competitor patterns." },
    { title: "24/7 Concierge Bot", status: "done", note: "Multilingual WhatsApp bot that qualifies buyers and routes hot leads." },
  ],
  experiments: [
    {
  ...
  docs: [
    {
      ...
      sections: [
        { h: "Principles", p: "Ship daily. Prove before you pitch. Measure what matters." },
        { h: "Stack", p: "Next.js, n8n, Meta Ads, Sheets, custom scrapers, GPT agents." },
      ],
    },
  ],
};
...
const Tag = ({ children }: any) => (
  <span className="px-2 py-0.5 text-[10px] uppercase border border-current mr-2 rounded-sm opacity-90">{children}</span>
);
        ...
        <ul className="mt-2 space-y-2 text-sm">
          {content.updates.map((u: any, i: number) => (
            <li key={i} className="flex items-start gap-2">
              ...
              <a href={u.url || "/"} className="hover:underline">{u.text}</a>
            </li>
          ))}
        </ul>
      </Row>
      <Row color="bg-black text-yellow-300">
        ...
        <div className="grid sm:grid-cols-2 gap-3 mt-2">
          {content.blog.slice(0, 4).map((p: any) => (
            <a key={p.id} href={`/blog`} className="border border-white/10 rounded-md p-3 hover:shadow">
        ...
        <ul className="text-sm mt-2 space-y-2">
          {content.experiments.slice(0, 3).map((e: any, i: number) => (
            <li key={i}>
              ...
              <a href="/experiments" className="hover:underline font-medium">{e.title}</a>
            </li>
          ))}
        </ul>
      </Row>
    ...
    <div className="mt-4 grid lg:grid-cols-3 gap-3">
      <div className="lg:col-span-1 space-y-2">
        {content.blog.map((p: any) => (
          <button key={p.id} onClick={() => setActive(p.id)} className={`w-full text-left border border-white/10 rounded-md p-3 hover:bg-white/5 ${active === p.id ? "ring-1 ring-white/30" : ""}`}>
      ...
      <Row key={i} color="bg-black text-cyan-300">
        <div className="flex items-start justify-between">
          ...
          <span className={`text-[10px] uppercase border border-current px-2 py-0.5 rounded-sm ${
            idea.status === "done" ? "text-green-300" : idea.status === "prototyping" ? "text-yellow-300" : "text-cyan-300"
          }`}>{idea.status}</span>
        </div>
      </Row>
    ))}
...
export default function TeletextApp({ initialPage = "100" }: { initialPage?: string }) {
  ...
  usePageDigits((digits) => {
    if (PAGES[digits]) navigate(digits);
  });
